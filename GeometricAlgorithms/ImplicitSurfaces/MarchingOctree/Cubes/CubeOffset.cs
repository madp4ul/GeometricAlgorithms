...
namespace GeometricAlgorithms.ImplicitSurfaces.MarchingOctree.Cubes
{
    class CubeOffset
    {
        ...
        public CubeOffset ToggleDimension(Dimension dimension)
        {
            return new CubeOffset(
        ...
        public CubeOffset SetValue(Dimension dimension, int value)
        {
            return new CubeOffset(
        ...
        public IEnumerable<Dimension> GetDifferingDemensions(CubeOffset other)
        {
            if (other.X != X)
            {
        ...
        public bool HasOnlyDifferencesOnDimensions(Dimension[] dimensions, CubeOffset other)
        {
        ...
        public static CubeOffset WithValueAtDimension(Dimension dimension, int valueAtDimension, int otherValue1, int othervalue2)
        {
            if (dimension == Dimension.X)
            {
                return new CubeOffset(valueAtDimension, otherValue1, othervalue2);
            }
            else if (dimension == Dimension.Y)
            {
                return new CubeOffset(otherValue1, valueAtDimension, othervalue2);
            }
            else if (dimension == Dimension.Z)
            {
                return new CubeOffset(otherValue1, othervalue2, valueAtDimension);
            }
            else
            {
                throw new ArgumentException();
        ...
        public static CubeOffset WithValuesAtDimension(
        ...
        {
            return obj is CubeOffset offset &&
                   X == offset.X &&
                   Y == offset.Y &&
                   Z == offset.Z;
        }
